...
package com.github.shareme.gwsmaterialuikit.library.advancerv.draggable;

import android.support.v7.widget.RecyclerView;
import android.view.ViewGroup;

import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.BaseSwipeableItemAdapter;
import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.RecyclerViewSwipeManager;
import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.SwipeableItemAdapter;
import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.SwipeableItemInternalUtils;
import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.action.SwipeResultAction;
import com.github.shareme.gwsmaterialuikit.library.advancerv.swipeable.action.SwipeResultActionDefault;
import com.github.shareme.gwsmaterialuikit.library.advancerv.utils.BaseWrapperAdapter;
import com.github.shareme.gwsmaterialuikit.library.advancerv.utils.WrapperAdapterUtils;
...
class DraggableItemWrapperAdapter<VH extends RecyclerView.ViewHolder> extends BaseWrapperAdapter<VH> implements SwipeableItemAdapter<VH> {
    ...
    public void onBindViewHolder(VH holder, int position, List<Object> payloads) {
        if (isDragging()) {
            ...
            if (itemId == draggingItemId && holder != mDraggingItemViewHolder) {
                ...
                if (LOCAL_LOGI) {
                    Timber.i("a new view holder object for the currently dragging item is assigned");
                }
    ...
    /*package*/ void onDragItemStarted(DraggingItemInfo draggingItemInfo, RecyclerView.ViewHolder holder, ItemDraggableRange range) {
        if (LOCAL_LOGD) {
            Timber.d("onDragItemStarted(holder = " + holder + ")");
        }
    ...
    /*package*/ void onDragItemFinished(boolean result) {
        if (LOCAL_LOGD) {
            Timber.d("onDragItemFinished(result = " + result + ")");
        }
    ...
    private void onDraggingItemRecycled() {
        if (LOCAL_LOGI) {
            Timber.i("a view holder object which is bound to currently dragging item is recycled");
        }
        mDraggingItemViewHolder = null;
    ...
    boolean canStartDrag(RecyclerView.ViewHolder holder, int position, int x, int y) {
        if (LOCAL_LOGV) {
            Timber.v("canStartDrag(holder = " + holder + ", position = " + position + ", x = " + x + ", y = " + y + ")");
        }
    ...
    boolean canDropItems(int draggingPosition, int dropPosition) {
        if (LOCAL_LOGV) {
            Timber.v("canDropItems(draggingPosition = " + draggingPosition + ", dropPosition = " + dropPosition + ")");
        }
    ...
    ItemDraggableRange getItemDraggableRange(RecyclerView.ViewHolder holder, int position) {
        if (LOCAL_LOGV) {
            Timber.v("getItemDraggableRange(holder = " + holder + ", position = " + position + ")");
        }
